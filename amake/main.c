...
**    $INITIAL:     Vrije Universiteit, The Netherlands. 1996
**    $CREATED:     ?
**    $VERSION:     1.02
**
**    $INFO:
...
static char *my_version="Amake Version 2.3";
...
Usage()
{
   ...
   PrintError("-i \t\tignore errors from subparts - try to complete cluster\n");
#ifdef USE_RSH
   ...
   PrintError("-V        \tprint version number and exit\n");
...
#ifdef DEBUG
    char *opt_string = "ihf:t:p:L:D:v*wnckrsCONGS%:?";
#else
    char *opt_string = "ihf:t:p:L:D:v*wnckrsCONGSV?";
#endif
    DBUG_ENTER("main");
    DBUG_PROCESS("amake");
