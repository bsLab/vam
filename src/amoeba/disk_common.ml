...
*)
let d_PHYS_SHIFT   = 9
let d_PHYSBLKSZ    = 1 lsl d_PHYS_SHIFT
...
*)
let max_PARTNS_PDISK =       8

(*
** Amoeba magic number
*)

let am_DISK_MAGIC =          0x0A30EBA0     
...
let sizeof_geom = 6 * int16_SIZE
let sizeof_part_tab = 2 * int32_SIZE + 2 * int16_SIZE + cap_SIZE 
let filler_SIZE = (d_PHYSBLKSZ - sizeof_geom)
                  - max_PARTNS_PDISK*sizeof_part_tab
                  - int32_SIZE - 2 * int16_SIZE - typeLEN
...
*)
let disk_REQBUFSZ = (29 * 1024)

let min_VBLK_SHIFT = d_PHYS_SHIFT
let max_VBLK_SHIFT = 30

(*
** Disk server structure. One for each device. 
*)
