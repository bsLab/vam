...
external putenv: name:string -> value:string -> unit = "unix_putenv"
...
external execv : prog:string -> 
                 args:string array -> 'a = "unix_execv"
external execve : prog:string -> 
                  args:string array -> 
                  env:string array -> 'a = "unix_execve"
external execvp : prog:string -> 
                  args:string array -> 'a = "unix_execvp"
external execvpe : prog:string -> 
                   args:string array -> 
                   env:string array -> 'a = "unix_execvpe"

external fork : unit -> int = "unix_fork"
external wait : unit -> int * process_status = "unix_wait"
external waitpid : wait_flag list -> pid:int -> 
                   int * process_status = "unix_waitpid"
...
external unix_readb : file_descr -> 
                      buffer -> 
                      int -> 
                      int -> 
                      int =  "unix_readb"
external unix_writeb : file_descr -> 
                       buffer -> 
                       int -> 
                       int -> 
                       int = "unix_writeb"
...
let read ~fd ~buf ~boff ~len =
  if len < 0 || boff + len > String.length buf
  then invalid_arg "Unix.read"
  else unsafe_read fd buf boff len

let write ~fd ~buf ~boff ~len =
  if len < 0 || boff + len > String.length buf
  then invalid_arg "Unix.write"
  else unsafe_write fd buf boff len

(*
** Bytebuf.buffer versions
*)

let readb ~fd ~buf ~boff ~len =
  unix_readb fd buf boff len

let writeb ~fd ~buf ~boff ~len =
  unix_writeb fd buf boff len
...
external lseek : fd:file_descr -> 
                 pos:int -> 
                 cmd:seek_command -> 
                 int = "unix_lseek"

external truncate : name:string -> 
                    size:int -> 
                    unit = "unix_truncate"

external ftruncate : fd:file_descr -> 
                     size:int -> 
                     unit = "unix_ftruncate"
...
external rename : oldname:string -> newname:string -> unit = "unix_rename"
external link : src:string -> dst:string -> unit = "unix_link"
...
external chmod : name:string -> perm:file_perm -> unit = "unix_chmod"
external fchmod : fd:file_descr -> perm:file_perm -> unit = "unix_fchmod"
external chown : name:string -> uid:int -> gid:int -> unit = "unix_chown"
external fchown : fd:file_descr -> uid:int -> gid:int -> unit = "unix_fchown"
...
external dup2 : src:file_descr -> dst:file_descr -> unit = "unix_dup2"
...
external mkdir : name:string -> perm:file_perm -> unit = "unix_mkdir"
...
external symlink : src:string -> dst:string -> unit = "unix_symlink"
external readlink : string -> string = "unix_readlink"
external mkfifo : name:string -> perm:file_perm -> unit = "unix_mkfifo"
external select :
  fds_in:file_descr list -> 
  fds_out:file_descr list -> 
  fds_exec:file_descr list -> 
  tmo:float ->
...
external lockf : fd:file_descr -> cmd:lock_command -> size:int -> unit 
            = "unix_lockf"
external kill : pid:int -> signal:int -> unit = "unix_kill"
...
external utimes : name:string -> time_acc:float -> time_mod:float -> unit = "unix_utimes"
...
external socket : domain:socket_domain -> 
...
external listen : fd:file_descr -> pend_req:int -> unit = "unix_listen"
...
let recv ~fd ~buf ~boff ~len ~flags =
  if len < 0 || boff + len > String.length buf
  then invalid_arg "Unix.recv"
  else unsafe_recv fd buf boff len flags

let recvfrom ~fd ~buf ~boff ~len ~flags =
  if len < 0 || boff + len > String.length buf
  then invalid_arg "Unix.recvfrom"
  else unsafe_recvfrom fd buf boff len flags

let send ~fd ~buf ~boff ~len ~flags =
  if len < 0 || boff + len > String.length buf
  then invalid_arg "Unix.send"
  else unsafe_send fd buf boff len flags

let sendto ~fd ~buf ~boff ~len ~flags ~addr =
  if len < 0 || boff + len > String.length buf
  then invalid_arg "Unix.sendto"
  else unsafe_sendto fd buf boff len flags addr
...
external tcsendbreak: 
    fd:file_descr -> 
    time:int -> 
    unit = "unix_tcsendbreak"

external tcdrain: file_descr -> unit = "unix_tcdrain"

type flush_queue = TCIFLUSH | TCOFLUSH | TCIOFLUSH

external tcflush: fd:file_descr -> queue:flush_queue -> unit = "unix_tcflush"

type flow_action = TCOOFF | TCOON | TCIOFF | TCION

external tcflow: fd:file_descr -> action:flow_action -> unit = "unix_tcflow"
...
let create_process ~prog ~args ~new_stdin ~new_stdout ~new_stderr =
  match fork() with
    0 ->
      begin try
...
let create_process_env ~prog ~args ~env ~new_stdin ~new_stdout ~new_stderr =
  match fork() with
    0 ->
      begin try
        perform_redirections new_stdin new_stdout new_stderr;
...
let open_proc_full prog env proc input output error toclose =
  match fork() with
     0 -> dup2 input stdin; close input;
          dup2 output stdout; close output;
          dup2 error stderr; close error;
          List.iter close toclose;
          execve "/bin/sh" [| "/bin/sh"; "-c"; prog |] env;
          exit 127
  | id -> Hashtbl.add popen_processes proc id

let open_process_full ~prog ~env =
  let (in_read, in_write) = pipe() in
  let (out_read, out_write) = pipe() in
  ...
  open_proc_full prog env (Process_full(inchan, outchan, errchan))
...
let close_process ~inout:(inchan, outchan) =
...
(*
** Some more utils from the Thread module
*)

let wait_timed_read ~fd ~tmo =
  match select [fd] [] [] tmo with ([], _, _) -> false | (_, _, _) -> true
let wait_timed_write fd ~tmo =
  match select [] [fd] [] tmo with (_, [], _) -> false | (_, _, _) -> true

(*external wait_signal : int list -> int = "caml_wait_signal"*)
let wait_signal il =
    invalid_arg "Unix.wait_signal: currently not implemented";
    -1
