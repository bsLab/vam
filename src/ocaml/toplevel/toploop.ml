...
(*
** Backtracing support for phrases from files ('use_file')
*)
...
open Instruct
external debug_add_compunit: debug_event array array -> 
                             (string*int) array -> unit
    = "debug_add_compunit"


(*
** Generic backtrace control for uncaught exceptions
*)

external print_backtrace: unit -> unit
    = "print_backtrace";;
external print_backtrace_mode: int -> unit
    = "print_backtrace_mode";;
...
let load_lambda ppf lam =
  ...
  let (init_code, fun_code) = Bytegen.compile_phrase !compunit_name slam in
  if !Clflags.dump_instr then
    fprintf ppf "%a%a@."
    Printinstr.instrlist init_code
    Printinstr.instrlist fun_code;

  let (code, code_size, reloc, events) = Emitcode.to_memory 
                                            init_code fun_code 
  in

  debug_events := !debug_events @ [(Array.of_list events)];
  debug_codes  := !debug_codes @ [code,code_size];
  ...
  *)
  ...
  with x ->
    ...
    *)
    print_backtrace ();     
    (*
    ** Switch off backtracing 
    *)
    print_backtrace_mode 0;
...
let use_file ppf name =
  ...
  try
    ...
    Clflags.debug := true;
    Line_of_pos_debug.init_line_of_pos filename false;
    ...
    let success =
      protecta Location.input_name filename (fun () ->
        try
          List.iter
            (fun ph ->
    ...
    close_in ic;
    debug_add_compunit (Array.of_list !debug_events) 
                       (Array.of_list !debug_codes);
...
let use_silently ppf name =
  protecta use_print_results false (fun () -> use_file ppf name)
