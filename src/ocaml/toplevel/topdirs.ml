...
(*
** Support for event backtracing from loaded modules.
*)
...
let load_compunit ic filename ppf compunit =
  ...
  (*
  ** Add event informations from this module to the backtrace
  ** event table if any (module must be compiled with '-g' option!).
  *)
  ...
  begin
    ...
    debug_add_compunit [|(Array.of_list evl)|] [|code,code_size|];
  end;
