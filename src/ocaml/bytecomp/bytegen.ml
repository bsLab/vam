...
let comp_primitive p args =
  ...
  | Pmatrixref(n, _, _) -> Kccall("matrix_get_" ^ string_of_int n, n + 1)
  | Pmatrixset(n, _, _) -> Kccall("matrix_set_" ^ string_of_int n, n + 2)
  ...
  | Levent(lam, lev) ->
      let event kind info =
        { ev_pos = 0;                   (* patched in emitcode *)
          ...
          ev_char = line_of_pos lev.lev_loc ;
          ev_kind = kind;
          ev_info = info;
...
let compile_phrase modulename expr =
  compunit_name := modulename;
  Stack.clear functions_to_compile;
  label_counter := 0;
  sz_static_raises := [] ;
