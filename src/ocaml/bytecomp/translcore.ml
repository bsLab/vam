...
let primitives_table = create_hashtable 57 [
  ...
  "%matrix_ref_1", Pmatrixref(1, Pmatrix_unknown, Pmatrix_c_layout);
  "%matrix_ref_2", Pmatrixref(2, Pmatrix_unknown, Pmatrix_c_layout);
  "%matrix_ref_3", Pmatrixref(3, Pmatrix_unknown, Pmatrix_c_layout);
  "%matrix_set_1", Pmatrixset(1, Pmatrix_unknown, Pmatrix_c_layout);
  "%matrix_set_2", Pmatrixset(2, Pmatrix_unknown, Pmatrix_c_layout);
  "%matrix_set_3", Pmatrixset(3, Pmatrix_unknown, Pmatrix_c_layout)
]
...
let transl_prim prim args =
  ...
  try
    ...
    begin match (p, args) with
      ...
      | (Pmatrixref(n, Pmatrix_unknown, _), arg1 :: _) ->
            let (k, l) = matrix_kind_and_layout arg1 in
            Pmatrixref(n, k, l)
      | (Pmatrixset(n, Pmatrix_unknown, _), arg1 :: _) ->
            let (k, l) = matrix_kind_and_layout arg1 in
            Pmatrixset(n, k, l)
      | _ -> p
    end
...
and transl_apply lam sargs =
  ...
  let rec build_apply lam args = function
      (None, optional) :: l ->
        ...
        let handle = protectc "func" lam
        and l = List.map (fun (arg, opt) -> may_map (protectc "arg") arg, opt) l
