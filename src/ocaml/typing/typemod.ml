...
let type_implementation sourcefile prefixname modulename initial_env ast =
  ...
  (*
  ** -o option now overides output names
  *)

  let objname = if (!Clflags.object_name = ("camlprog" ^ Config.ext_obj)) 
                    then    
                        (* not set; use prefix *)
                        prefixname ^ ".cmi" 
                    else
  ...
  let is_in_path path file =
    try ignore(find_in_path path file); true; with
    | Not_found -> false;    
    in

  let coercion =
    if is_in_path !Config.load_path (prefixname ^ !Config.interface_suffix) then begin
      let intf_file =
        try find_in_path !Config.load_path objname
        with Not_found -> objname in
      ...
      Env.save_signature sg modulename objname;
      Tcoerce_none
    end in
  (str, coercion)
