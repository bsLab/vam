...
**    $INITIAL:     (C) 1996 projet Cristal, INRIA Rocquencourt
**    $CREATED:     ?
**    $VERSION:     1.04
**
**    $INFO:
...
let progerr s = raise (Failure ("Programming Error: "^s))
...
let get_some op =
    match op with
    | Some s -> s;
    | None -> progerr "get_some but None!"
...
external file_size : string -> int = "caml_file_size"
