...
external get_sys_config: unit -> string * string * string * string * int 
        = "sys_get_sys_config"

let argv = get_argv()
let (os_type, word_size) = get_config()
let (os_type, os_id, arch_type, model_type, word_size) = get_sys_config()
...
external clock: unit -> float = "sys_clock"
