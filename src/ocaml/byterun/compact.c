...
**    $INFO:
**
**  Heap compaction, part of the Gargabe Collector
**
**    $ENDOFINFO
**
*/
    ...
    if (Ecolor (q) == 0 && Is_in_heap (q))
    {
        switch (Ecolor (Hd_val (q)))
        {
            ...
            case 1: /* Infix header: make inverted infix list. */
                /*
                ** Double inversion: the last of the inverted infix list 
                ** points to the next infix header in this block.  The last
                ** of the last list contains the original block header. 
                */
                {
                    ...
                    if (Tag_ehd (*hp) == Closure_tag)
                    {
                        ...
                        *p = *hp;
                        ...
                        /*
                        ** Change block header's tag to Infix_tag, 
                        ** and change its size to point to the infix list. 
                        */
                    ...
                    {                            
                        ...
                        /*
                        ** Point the last of this infix list to the current 
                        ** first infix list of the block. 
                        */
                                      ...
                                      /* in bytes, including header */
{
    ...
    while ((Chunk_size (compact_fl) - Chunk_alloc (compact_fl))
             <= Bhsize_wosize (3)
            ...
            while ((char *) p < chend)
            {
                ...
                if (Ecolor (q) == 0 || Tag_ehd (q) == Infix_tag)
                {
                    ...
                    *p = Make_header (Wosize_whsize (sz), t, Caml_white);
                    ...
                    {
                        /*
                        ** Rebuild the infix headers and revert the infix 
                        ** pointers. 
                        */
                        while (Ecolor ((word) infixes) != 3)
                        {
                            infixes = (word *) 
                                        ((word) infixes & ~(unsigned long) 3);
                            q = *infixes;
                            while (Ecolor (q) == 2)
                            {
                                word next;
                                ...
                                next = * (word *) q;
                                *(word*)q=(word) Val_hp 
                                            ((word *) newadr + (infixes - p));
                                q = next;
                            }                    
                            Assert (Ecolor (q) == 1 || Ecolor (q) == 3);
                            *infixes = Make_header 
                                        (infixes - p, Infix_tag, Caml_white);
                            infixes = (word *) q;
                        }
                    }
                    p += sz;
                ...
                {                                        
                    Assert (Ecolor (q) == 3);
                    /*
                    ** This is guaranteed only if compact_heap was called 
                    ** after a nonincremental major GC:
                    **       
                    ** Assert (Tag_ehd (q) == String_tag);
                    **
    ...
    */
    {
        ...
        while (ch != NULL)
        {
            if (Chunk_size (ch) > Chunk_alloc (ch))
            {
                ...
                *p = Make_header (Wosize_bhsize 
                                    (Chunk_size (ch) - Chunk_alloc (ch)),
                                    0, Caml_white);
