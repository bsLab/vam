...
#if !macintosh && !_WIN32 && !defined(AMOEBA_RAW)
  #include <sys/wait.h>
#endif
...
/*
** Calendar time or system clock in seconds.
*/

CAMLprim value sys_clock(value unit)
{
#ifdef HAS_GETTIMEOFDAY
  struct timeval tv;
  gettimeofday(&tv, NULL);
  return copy_double(tv.tv_sec);
#else
  return copy_double((int)time (NULL));
#endif
}
...
** OS type id string
*/
char *ocaml_os_type=OCAML_OS_TYPE;
