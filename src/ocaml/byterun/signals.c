...
**    $INFO:
** 
**  Signal and enter/leave blocking section handlers.
**
**  Note for AMOEBA_RAW: UNIX signals are not supported in this
**  limited process environment. Most fucntions here exist only 
...
{
  value res;
#if defined(POSIX_SIGNALS) && !defined(AMOEBA_RAW)
  sigset_t sigs;
  /*
...
#if !defined(POSIX_SIGNALS) && !defined(BSD_SIGNALS) && !defined(AMOEBA_RAW)
