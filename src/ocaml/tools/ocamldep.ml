...
(*
** Last modified by 
**      Stefan Bosse 30/04/05
**
** Added: 
**          -- os_type "Amoeba"
**          -- new -amake option: output dependencies for Amake 
**          -- added -dep <path> option: adds each include source to
**             the dependency list
**
*)                            
...
(* output amake dependencies *)
let amake_only = ref false 
let add_includes = ref []
...
(*
** Convert .cmo and .cmi to .ml and .mli extensions. Only needed
** in amake mode.
*)

let conv_dep_name dep =
    let strlen = String.length dep in
    let ext = String.sub dep (strlen - 3) 3 in
    match ext with
    | "cmx" -> "";
    | "cmi" -> (String.sub dep 0 (strlen - 3))^"mli";
    | "cmo" -> (String.sub dep 0 (strlen - 3))^"ml";
    | _     -> dep


let (depends_on, escaped_eol) =
  if (!amake_only = false) then 
      match Sys.os_type with
      | "Unix" | "Amoeba" | "Win32" | "Cygwin" -> (": ", "\\\n    ")
      | "MacOS" -> ("\196 ", "\182\n    ")
      | _ -> assert false
  else
        ("","\n")
...
let print_dependencies target_file deps =
  ...
  | _ ->
    ...
    let rec print_items pos = function
      [] -> if (!amake_only = false) then print_string "\n" 
    | dep :: rem ->
        if (!amake_only = false) then
        begin
        ...
        begin
            (* convert extensions *)
            let convdep = conv_dep_name dep in
            if (convdep <> "") then
            begin
                print_string convdep;print_newline ();
                print_items (String.length convdep + 5) rem        
            end
        end in    
    ...
    try
      ...
      try
        if Filename.check_suffix source_file ".ml" then begin
          ...
          print_dependencies (basename ^ ".cmo") !add_includes;
        end else
        if Filename.check_suffix source_file ".mli" then begin
          ...
          print_dependencies "" !add_includes;
        end else
          ();
...
let usage = "Usage: ocamldep [-I <dir>] [-native -amake] [-dep <path> []] <files>"
...
let _ =
  ...
  Arg.parse [
     ...
     "-native", Arg.Set native_only,
       ...
       (no .cmo files)";
     "-amake",  Arg.Set amake_only,
       "  Generate dependencies for Amake (only for one file) ";
     "-pp", Arg.String(fun s -> preprocessor := Some s),
       "<command>  Pipe sources through preprocessor <command>";
     "-dep", Arg.String(fun s -> add_includes := !add_includes @ [s]),
       "<path> Add this path to the dependency list";
