...
** layer! This block size is always used for the inode virtual disk.
*)
...
(*
** Default Partition names and paths in the kernel root directory
*)

let def_vpath = "/"
let def_vdisk_inode = "vdisk:02"
let def_vdisk_data  = "vdisk:04"
...
(*
** Create the filesystem (two vdisk partitions). 
**
** Args:
...
**  vdisk_data:   Path and name for the data vdisk
**  
** Return:
**  status
**  afs_super 
...
let create_vdisk_fs
    ...
    begin
        ...
        *)
        ...
        let stat2,vdisk_data_cap=name_lookup vdisk_data in

        if stat1 <> std_OK then raise (Error stat1);
        if stat2 <> std_OK then raise (Error stat2);
            
        (*
        ** Is there already an existing file system ?
        *)
        ...
        begin
            ...
            *)
            ...
            sys_log Sys_info "AFS: Checking for existing filesystem... "; 

            let superb = buf_create def_block_size in
            ...
            begin
                sys_log Sys_fatal "exists already. Set the overwrite flag.\n";
        ...
        *)
        ...
        let super1 = buf_create def_block_size in
        let empty2 = buf_create def_block_size in
        ...
        *)
        ...
        Bytebuf.fill super1 ~pos:0 ~len:32 0xaa;
        ignore (buf_put_string ~buf:super1
                               ~pos:0
                               ~str:magic_str1);


        Bytebuf.fill empty2 ~pos:0 ~len:32 0xaa;
        ignore (buf_put_string ~buf:empty2
                               ~pos:0
                               ~str:magic_str2);
        ...
        Bytebuf.fill super1 ~pos:soff ~len:480 0xaa;
        Bytebuf.fill empty2 ~pos:soff ~len:480 0xaa;


        let pos = buf_put_string ~buf:super1 
                                 ~pos:soff ~str:label in
        let pos = buf_put_int32  ~buf:super1
                                 ~pos:(soff+256) ~int32:ninodes in
        let pos = buf_put_int32  ~buf:super1
        ...
        if (stat <> std_OK) then
            raise (Error stat);
        let stat = disk_write vdisk_data_cap ~start:0
                                         ~num:1
                                         ~blksize:def_block_size
                                         ~buf:empty2  
                                         ~pos:0 in
        ...
        *) 
        ...
        do
            ...
            do
                inode.i_file_num <- j + i*ninodes_block;
        ...
        *)
        ...
        let live_table = buf_create (ceil_block_bytes live_size def_block_size) in
        ...
        ** blocks
        *)
        let live_block_off = 1 + (ceil_block (ninodes*inode_SIZE) def_block_size) in
        ...
        (*
        ** Write the livetime table to vdisk
        *)
        let live_write lv =
            (*
...
(*
** Start the file system server (two virtual disk)
**
**  vdisk_inode:  Path and name for the inode vdisk 
**  vdisk_data:   Path and name for the data vdisk
...
let start_vdisk_fs 
    ...
    begin
        ...
        *)
        ...
        (*
        ** Here we store the inodes and the data in two virtual disks.
        *)

        sys_log Sys_start "AFS: Checking virtual disks...\n"; 
        ...
        *)
        ...
        let live_block_off = ceil_block (512 + (super.afs_nfiles*inode_SIZE))
                                   def_block_size in
        let live_block_size = ceil_block live_size def_block_size in
        ...
        let live_set lvt ~obj ~time ~flag =
#ifdef DEBUG
            Db.Pr.sddd 100 "live_set obj,val,flag" obj time flag;
#endif
            buf_set lvt obj ((time land 0x7f) lor
                                ((flag lsl 7) land 0x80))
        in             
        ...
        *)
        let def_res_SIZE = ceil_block_bytes def_res_SIZE block_size in
...
#ifdef DEBUG
            Db.Pr.sddd 1 "server:read_inode: [baddr,bsize,size]" 
                    addr bsize size;
#endif
            let stat = 
                disk_read vdisk_inode_cap 
        ...
        *)
        ...
        let read_data   ~obj
                        ...
                        ~size
            =
            let bsize =  ceil_block size block_size in

#ifdef DEBUG
            Db.Pr.sdd 0 "server:read_data: [baddr,bsize]" addr bsize;
#endif
            let stat = 
                disk_read vdisk_data_cap 
                                  ~start:addr
                                  ~num:bsize
                                  ~blksize:def_block_size
                                  ~buf:data
...
#ifdef DEBUG
            Db.Pr.sdd 2 "server:write_data: [baddr,bsize]" addr bsize;
#endif
            let stat = 
                disk_write vdisk_data_cap 
                                  ~start:addr
                                  ~num:bsize
                                  ~blksize:def_block_size
                                  ~buf:data
        ...
        *)
        ...
        if (stat <> std_OK) then
        begin
            sys_log Sys_fatal "AFS: can't create inode cache\n";
            raise (Error stat);
        end;
        ...
        *)
        ...
        if (stat <> std_OK) then
        begin
            sys_log Sys_fatal "AFS: can't create data cache\n"; 
            raise (Error stat);
        end;
...
#ifdef DEBUG
            Db.Pr.sdd 0 "server:read_inode: [baddr,bsize]" addr bsize;
#endif
            let stat = 
                disk_read vdisk_inode_cap 
                                  ~start:addr
