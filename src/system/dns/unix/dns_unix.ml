...
**    $INFO:
**
**  Unix implementation of the Directory and Name Amoeba server.
**
**    $ENDOFINFO
**
*)
...
let buf_get_row ~buf ~pos ~ncols =
    ...
    begin
        sys_log Sys_warn "DNS: Warning: ncols=%d rowname=%s\n" ncols r1;
    end;
...
let def_lpath = "/amoeba/dns"
let def_dns_part = "ldisk:02"
...
**  dir_part:   Partition path and name
...
let create_unix_fs
    ...
    begin
        ...
        *)
        ...
        let magic = buf_create 32 in
    
        Bytebuf.fill magic ~pos:0 ~len:32 0xaa;
        ignore (buf_put_string ~buf:magic
                               ~pos:0
                               ~str:magic_str);
        ...
        *)
        ...
        let super  = buf_create 480 in

        Bytebuf.fill super ~pos:0 ~len:480 0xaa;
        ...
        let pfd = Unix.openfile dir_part
        ...
        sys_log Sys_info  "DNS: Writing partition magic headers... "; 

        let wrn = Unix.writeb pfd magic 0 32 in
        if (wrn <> 32) then
        ...
        let wrn = Unix.writeb pfd super 0 480 in
        if (wrn <> 480) then
            raise (Error std_IOERR);
        ...
        let lsn = Unix.lseek pfd 512 SEEK_SET in
        ...
        *)
        ...
        do
                ...
                let wrn = Unix.writeb pfd buf 0 inode_SIZE in
        ...
        *)
        ...
        let live_write lv =
            ...
            *)
            ...
            let live_off = 512 + (super.dns_ndirs*inode_SIZE) in
            let n = Unix.lseek pfd live_off SEEK_SET in
            let wrn = Unix.writeb pfd lv 0 live_size in
        ...
        let write_inode ~obj
            ...
            begin
                ...
                *)
                ...
                let off' = (off / blocksize) * blocksize in
                let off'' = off mod blocksize in


                let n = Unix.lseek pfd off' SEEK_SET in
                let n = Unix.readb pfd buf 0 blocksize in  

                blit_bb ~src:data ~src_pos:0 
                        ~dst:buf ~dst_pos:off'' ~len:size;

                let n = Unix.lseek pfd off' SEEK_SET in
                let n = Unix.writeb pfd buf 0 blocksize in
...
**
**  dir_part:   Directory Mapping Partition path and name
...
let start_unix_fs 
    ...
    begin
        ...
        *)
        ...
        (*
        ** Here we store the inodes in one file partition.
        *)

        sys_log Sys_start  "DNS: Opening partition...\n"; 

        sys_log Sys_start "DNS: Inode part -> %s\n" dir_part;
        let part_fd = Unix.openfile dir_part [O_RDWR] 384 in
        ...
        *)
        ...
        let stat = 
            try
            begin
                let _ = Unix.lseek part_fd 0 SEEK_SET in
                let n = Unix.readb part_fd superb 0 512 in
        ...
        *)
        ...
        let live_read lv =
          ...
          begin
            let live_off = 512 + (super.dns_ndirs*inode_SIZE) in
            let n = Unix.lseek part_fd live_off SEEK_SET in
            let n = Unix.readb part_fd lv 0 live_size in
            ...
            begin
                ...
                *)
                ...
                let n = Unix.lseek part_fd live_off SEEK_SET in
                let n = Unix.writeb part_fd lv 0 512 in
                if ( n <> 512) then
                    std_IOERR
                else
                    std_OK
        ...
        let live_write lv =
          ...
          begin
            ...
            *)
            ...
            let live_off = 512 + (super.dns_ndirs*inode_SIZE) in
            let n = Unix.lseek part_fd live_off SEEK_SET in
            let wrn = Unix.writeb part_fd lv 0 live_size in
            if (wrn <> live_size) then
                std_IOERR
            else
                std_OK
        ...
        *)
        ...
        sys_log Sys_start " OK.\n"; 
        sys_log Sys_start "DNS: Creating directory and inode caches... \n"; 
...
#endif
                let n = Unix.lseek part_fd off SEEK_SET in
                let n = Unix.readb part_fd data 0 size in
...
#endif
                let n = Unix.lseek part_fd off SEEK_SET in
                let n = Unix.writeb part_fd data 0 size in
                if ( n = size) then 
                    std_OK
                else
                    std_SYSERR
        ...
        *)
        ...
        let cache_data = Cache.create ~size:cache.c_dir_buffers
            in
    ...
    with
        ...
        | Buf_overflow -> 
                sys_log Sys_fatal "\nDNS: Abort: Buffer Overflow\n"; 
                std_OVERFLOW,nildnsserver
        | _ -> 
                sys_log Sys_start "\nDNS: Abort: Input-Output Error\n"; 
                std_IOERR,nildnsserver
