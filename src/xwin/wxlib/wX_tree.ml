...
** Changed [@SB]:
**
**  -- method set_notify added
**  -- size of branch graphic increased
**
*)
...
class t parent attributes =
  ...
  method size_request =     
    ...
    let (dx,dy) = iter_size desc 14  (2 * w.w_ipad_y) in
(*    List.iter (fun leaf ->
        let sz1 = leaf.leaf_widget#size_request in
        let borders = 2 * sz1.border_width  in
        leaf.leaf_dx <- sz1.requested_width + leaf.leaf_x + borders;
        ...
        dx := max !dx (max leaf.leaf_dx 
            (if leaf.leaf_closed then 0 else leaf.leaf_dx'));
        dy := !dy + (max 14 leaf.leaf_dy) + 
          (if leaf.leaf_closed then 0 else leaf.leaf_dy');
    ) desc;
  *)
    sz.requested_width <- max (dx + (2 * w.w_ipad_x) + 14) sz.min_width;
  ...
  method size_allocate x y dx dy =
    ...
    let rec iter desc =
      ...
      | leaf :: tail ->
          let sz1 = leaf.leaf_widget#size_request in
          let borders = 2 * sz1.border_width  in
          let y' = !y + max 0 (14 - leaf.leaf_dy) in
          leaf.leaf_widget#size_allocate 
                    (x+leaf.leaf_x+sz1.border_width) 
                    (y'+sz1.border_width) 
                    (sz1.requested_width) 
                    (leaf.leaf_dy - 2 * borders);
          y := !y + max 14 leaf.leaf_dy;
          let _ =
            match leaf.leaf_subtree with
  ...
  method refresh =
    ...
    let rec iter desc y dy =
      match desc with
        [] -> ()
      | leaf :: tail ->
          let h = max leaf.leaf_dy 14 in
          let mid = h/2 in
          match leaf.leaf_subtree with
            None ->
              let y' = y+dy+mid in
              draw [7, y, 7, y'; 7, y', (14+leaf.leaf_x), y'];
              iter tail y' (h - mid)
          | Some o2 ->
              let y' = y + dy + mid in
              let y1 = y' - 5 in
              ...
              if leaf.leaf_win == noWindow then
                begin
                  leaf.leaf_win <- X.createWindow s.s_display w.w_window
                    0 (y1-mid) 13 (y1+mid-1) copyDepthFromParent InputOnly 
              ...
              iter tail y2 (h - mid - 5 + (
                  if leaf.leaf_closed then 0 else leaf.leaf_dy'))
    in
    iter desc w.w_ipad_y 0

    (*
    ** Set user notify function for open/close events on branches
    *)
