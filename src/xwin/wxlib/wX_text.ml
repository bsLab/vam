...
**    $INFO:
**
**  Text String widget (also lines of strings).
**
**    $ENDOFINFO
**
*)
  ...
  sig
    ...
    class t :
      ...
      object
        ...
        method set_font : WX_types.font -> unit
...
module Make(Text: Text)  = struct
    ...
    class t parent (stext : tree tree_desc) attributes =
      object (self)
          ...
          if not w.w_size_modified  || sz.comp_timestamp = s.s_timestamp then sz else 
            begin
              ...
              and lines_size text y =
                ...
                for i = 0 to n - 1 do
                  let (repr,rpos) = representation text.tree_text lines.(i) in
                  ...
                  for j = 0 to ni - 1 do
                    match line.(j) with
                      String (attr, len) ->
                        ...
                        dx := !dx + (string_width font repr);
                        dy := max !dy font.font_height
                    | RealString (attr, str) ->
                        let font = get_font font attr in
                        dx := !dx + (string_width font str);
                        dy := max !dy font.font_height
                    | Widget w ->
                        (*
                        ** Multiple displays of a single text is 
                        ** not implememeted 
                        *)
                        let w = w.(0) in
                  ...
                  if !dy <> text.line_height then
                    if n = 1 || i = 0 then text.line_height <- !dy else
                      begin
                        ...
                        (* 
                        ** Problem, this widget requires a greater height 
                        ** than expected.
        ...
        (*
        ** Direct call to #refresh is not allowed. 
        ** Use #wait_refresh instead 
        *)
                  ...
                  for i = 0 to nn - 1 do
                    ...
                    match item with
                      Widget w -> 
                        ...
                        x := !x + w#width
                        (* 
                        ** do not redraw widget since they receive 
        ...
        (* 
        ** 't' is a simple text (an array of lines (array) of 
        ** text items (array) 
        *)  
...
class of_file parent filename attributes =
  ...
  inherit SimpleTree.t parent (file_to_stext filename) 
		([
            Relief ReliefSunken; 
            Background "white";
            IpadX 2; IpadY 2 
...
class of_string parent lines attributes =
  ...
  inherit SimpleTree.t parent (make_simple_text lines)
		([
            IpadX 5; IpadY 5
        ] @ attributes)
  ...
  method set_string str =
    self#set_text (make_simple_text str)
end
