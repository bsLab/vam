...
** Changes:
**
**  -- page_up/down scrolls now only 90% of the page height; it's more
**     user friendly !
**  -- update_x/y: removed view.x/y correction (something wrong with him)
** 
**
*)
...
*)
  ...
  method page_up = 
    ...
    adj.v_pos <- max (adj.v_pos - (adj.v_page*9)/10) 0; 
  ...
  method page_down =
    let old_vpos = adj.v_pos in    
    adj.v_pos <- min (adj.v_pos + (adj.v_page*9)/10) (v_total - adj.v_page);
    if old_vpos <> adj.v_pos && validate () then
      self#update_all
    else
      adj.v_pos <- old_vpos
