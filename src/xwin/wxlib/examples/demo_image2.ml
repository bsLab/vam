(*
** Example desktop with buttons, editable lines and a
** gray image.
*)
...
let imagebuf = String.create (iwidth*iheight)   
...
let fillgray () =
        ...
        do
            for x = 0 to iwidth-1 
            do
                imagebuf.[y*iwidth+x] <- (char_of_int (5*(x mod 50))) ;
            done;
        done
        ...
        do
            ...
            do
                imagebuf.[y*iwidth+x] <- char_of_int 
                    ( 
                        255-(int_of_char imagebuf.[y*iwidth+x])
                    )
				 	;
            done;
        done
...
class topwx root attr =
    ...
    let rand_but = new WX_button.with_label hbar#container " INVERT " 
                        [ IpadX 4; IpadY 2] in
