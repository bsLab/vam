...
let _ =
    ...
    __(thread_create (fun () ->
        ...
        do
            ...
            __(thread_delay 2 SEC);
            let last = log1#last_row in
            let line = log1#get_line last in
            Printf.printf "last_row=%d" last; print_newline();
            log1#modify_line last ("Modified..."^line);
        done;
        ) ());
    ...
    __(top#configure [Bindings [
        EnterWindow,(fun _ -> 
            ...
            begin
                printed := true;
                print_eps "log3.eps" 1.0 vbox#contained;
            end;
            );
        ]]);
