...
class virtual t screen = 
    ...
    (*
    ** Allocate a new font specified with text_font, text_style and
    ** size in pixel.
    *)
    method font_make name style size safe =
        let font_name = VX_common.font_name name style size in
        ...
        try
            ...
            let font = { 
                font_class = name;
                font_style = style;
                font_size = font_size size; (* actual size *)
                font_name = font_name;
              ...
              let color =
                ...
                | Namedcolor c ->
                    ...
                    begin
                        ...
                        let r = 255 * r in
                        let g = hex c.[4] + 16 * hex c.[3] in 
                        let g = 255 * g in
                        let b = hex c.[6] + 16 * hex c.[5] in 
                        let b = 255 * b in 
            ...
            let pixmap = 
                ...
                | FromFile file ->
                    let file = if Filename.is_relative file then
                                    Utils.find_in_path !pixmap_path file 
                               else file
                        in
                    Xpm.createPixmapFromFile display scr.scr_root
                                             scr.scr_default_colormap 
                                             scr.scr_root_depth file;
                ...
                | FromData data ->
                    Xpm.createPixmapFromData display scr.scr_root
                                             scr.scr_default_colormap 
                                             scr.scr_root_depth data;
                in
            Hashtbl.add s.s_pixmaps name pixmap;
            pixmap
  
    method  pixmap_get name =
        let s = self#screen in
        Hashtbl.find s.s_pixmaps name 

    method  image_make (name,imagemap) =
        let s = self#screen in
        try     
            Hashtbl.find s.s_images name    
        with Not_found ->
            let scr = s.s_screen in
            let display = s.s_display in
            let imagemap = 
                match imagemap with
                | FromGrayPacked imdes ->
                    let (buf,width,height) = imdes in
                    Ximage.createImageFromGrayPacked 
                        display scr.scr_root scr.scr_default_colormap 
                        width height scr.scr_root_depth buf;
                | FromGrayBuf imdes ->
                    let (buf,width,height) = imdes in
                    Ximage.createImageFromGrayBuf 
                        display scr.scr_root scr.scr_default_colormap 
                        width height scr.scr_root_depth buf;
                | FromGrayMatrix imdes ->
                    let (mat,width,height) = imdes in
                    Ximage.createImageFromGrayMatrix 
                        display scr.scr_root scr.scr_default_colormap 
                        width height scr.scr_root_depth mat;
                | FromRGBPacked imdes ->
                    let (buf,width,height) = imdes in
                    Ximage.createImageFromRGBPacked 
                        display scr.scr_root scr.scr_default_colormap 
                        width height scr.scr_root_depth buf;
                | FromRGBMatrix imdes ->
                    let (mat,width,height) = imdes in
                    Ximage.createImageFromRGBMatrix 
                        display scr.scr_root scr.scr_default_colormap 
                        width height scr.scr_root_depth mat;
                in
            Hashtbl.add s.s_images name imagemap;
            imagemap
  
    method  image_get name =
        let s = self#screen in
        Hashtbl.find s.s_images name 
            ...
            let cursor = 
                ...
                | BitmapCursor (bitmap,mask) ->
                    ...
                    let (w,h,hot_x,hot_y,pix) = 
                        Xpm.createBitmapFromFile display 
                                                 screen.scr_root 
                                                 bitmap in
                    try
                        let (ww,hh,_,_,mask) = 
        ...
        try
            ...
            begin
                ...
                if abs(qp.qp_root_x - old_x) >= !delta_move_size ||
                   abs(qp.qp_root_y - old_y) >= !delta_move_size 
                    then Simple
                    else DeltaMove
            end
        with _ -> Other
