...
**
**  Last modified:  23/10/01
**
**  Changes:    -- XKB  modifier fix added
**              
**
*)
...
(*
** Add your local key symbol table (for example germanSysms) 
** to this list to eliminate dead keys.
*)

let locale_keysyms = ref []
...
let uKeyInitialize display =
  if display.dpy_keysyms = [||] then
    begin
      ...
      (*
      ** Init local key symbol tables if any
      *)

      List.iter (fun kl ->
                  let (keysym,modmask,str) = kl in
                  let p =
                  {
...
(*
** Translate a keysymbol to ascii char. Keep in mind the current modifier
** mask.
*)
...
let addKeysym display keysym modmask str =
  uKeyInitialize display;
  let p =
    {
      key_string = String.copy str;
...
let addKeysyms display key_list  =
  
  List.iter (fun kl ->
                let (key,mask,str) = kl in
                  addKeysym display key mask str
             ) key_list
...
(*
** Translate a key code symbol in respect to the modifier mask
** (shift, ctrl, ...)
*)
...
let modifier_fix display xev =

    match xev.ev_event with
      | KeyPressEvent t -> 
            (*
            ** Look for the keycode of the Mod1..Mod5 keys
            *)
            ...
            let map = display.dpy_modifiermap in
            for i=3 to (Array.length map) - 1 
            do
                if (t.Xkey.detail = (map.(i).(0)) ) then
                begin
                    display.dpy_modifier_mask <-
                        display.dpy_modifier_mask lor 
                        (1 lsl i);
                end;
            done;
            {
                ev_num = xev.ev_num;
                ev_type = xev.ev_type;
                ev_serial_out = xev.ev_serial_out;
                ev_serial_in = xev.ev_serial_in;
                ev_sent = xev.ev_sent;
                ev_window = xev.ev_window;
                ev_event = KeyPressEvent {
                                Xkey.detail = t.Xkey.detail;
                                Xkey.time = t.Xkey.time;
                                Xkey.root = t.Xkey.root;
                                Xkey.event = t.Xkey.event;
                                Xkey.child = t.Xkey.child;
                                Xkey.x_event = t.Xkey.x_event;
                                Xkey.y_event = t.Xkey.y_event;
                                Xkey.x_root = t.Xkey.x_root;
                                Xkey.y_root = t.Xkey.y_root;
                                Xkey.state = t.Xkey.state lor
                                        display.dpy_modifier_mask;
                                Xkey.same_screen = t.Xkey.same_screen;
                            }
            } 
      ...
      | KeyReleaseEvent t -> 
            ...
            *)
            ...
            do
                ...
                begin
                    display.dpy_modifier_mask <-
                        display.dpy_modifier_mask land
                        (lnot (1 lsl i));
                end;
            done;
            {
...
let lookupString display event =
  ...
  let (modifiers,keysym) =
    uTranslateKey display kev.Xkey.detail 
                  (kev.Xkey.state lor display.dpy_modifier_mask)
  in
    (uTranslateKeySym display keysym 
                      (kev.Xkey.state lor display.dpy_modifier_mask), 
                      keysym, modifiers)
