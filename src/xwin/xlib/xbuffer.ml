...
**              -- brocken connection now don't raise an exception 
**              -- XKB modifier fix
**              -- some fixes and changes on display structure and
**                 getXServerInfo
**              -- unsafe_chr fix (now argument land 0xff)
**              -- moved OS dependent stuff to Xos
**
*)
...
let unsafe_chr = Char.chr
*)

  (* les cardinaux *)
let setCard8 buffer pos int = 
  unsafe_set buffer pos (unsafe_chr (int land 0xff))
let setCard16 buffer pos int =
  unsafe_set buffer pos (unsafe_chr (int land 0xff));
  unsafe_set buffer (pos+1) (unsafe_chr ((int asr 8) land 0xff))

let setCard32 buffer pos int =
  unsafe_set buffer pos (unsafe_chr (int land 0xff));
  unsafe_set buffer (pos+1) (unsafe_chr ((int asr 8) land 0xff));
  unsafe_set buffer (pos+2) (unsafe_chr ((int asr 16) land 0xff));
  unsafe_set buffer (pos+3) (unsafe_chr ((int asr 24) land 0xff))
  (*L*)
...
let setEnum8 buffer pos valeur = 
  unsafe_set buffer pos (unsafe_chr ((int_of_enum valeur) land 0xff))
...
let getXServerInfo b chan maj min screen_num server_name =
...

  {
    ...
    wrappers_queue = Xfifo.create (Obj.magic "");
    event_queue = Xequeue.create (Obj.magic "");
    ...
    (*
    ** Don't raise an exception - we need thread cleanup 
    *)
    dpy_broken = (fun () -> 
        print_string "Broken connection to server.\n";);
    ...
    (* Connection status *)
    dpy_conn_status = Connection_active;
  }
