...
type t = {
    ...
    (* functions to set and get the pixel color index in the zpixmap *)
    setPixel : (t -> coord -> coord -> pixel -> unit);
    getPixel : (t -> coord -> coord -> pixel);

    (* was the image bitmap modified ? *)
    mutable modified : bool 
  }

(*
** The number of bits per pixel is a mutiple of 8
*)
...
(*
** The number of bits per pixel is 1 (bitmap)
*)
...
(*
** The number of bits per pixel is neither 1 nor 8
*)
...
(*
** Transform the image [image] to and from the X pixmap format [raw_image], 
** mainly byte order manipulation.
*)
...
(*
** Support for the translate_bits function
*)
