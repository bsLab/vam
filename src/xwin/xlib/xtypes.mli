...
**
**  Changes:	-- XKB fix (correct keyboard  modifier handling)
**              -- some fixes and changes on display structure 
**              -- OS independent connection channel (Xos module)
**
*)
                                                                     
type x_channel = X_channel of int
...
and display =
  { 
    (*
    ** Main X display strucure. Members in order of the X protocol
    ** requirement.
    *)

    mutable comm_channel: x_channel;

    mutable serial_out: int;
    mutable serial_in: int;
    
    wrappers_queue: (int * (string->unit)) Xfifo.t;
    event_queue: xevent Xequeue.t;
    ...
    (*
    ** Connection status: Connection_active | Connection_closed
    *)
    mutable dpy_conn_status: connection_status;
...
val real_time : int * int -> float
val anyButton : int
val copyVisualFromParent : int
val pixmapParentRelative : int
val copyPixmapFromParent : int
val copyColormapFromParent : int
val noCursor : int
