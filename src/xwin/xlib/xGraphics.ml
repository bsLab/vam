...
let draw_rect x y dx dy = 
  let v = getview () in
  (match !update_style with
    ImmediateDraw -> Xlib.drawRectangle v.display v.window v.gc x y dx dy;
  | FlushAll -> ()
  | FlushClipped ->
      updates := (x,y,dx,dy) :: !updates;
      );
  Xlib.drawRectangle v.display v.store v.gc x y dx dy
