...
let simpleWindowRequest (op : requestOpcode) (w : window) = 
  simpleRequest op w
let simpleFontRequest  (op : requestOpcode) (w : font) = 
  simpleRequest op w
let simpleColormapRequest  (op : requestOpcode) (w : colormap) = 
  simpleRequest op w

external obj_tag : 'a -> int = "obj_tag" "noalloc"
  
let rec insert tag v res =
  match res with
    [] -> [v]
  | head :: tail ->
      if tag > obj_tag head then 
...
let grabPointerReq win owner_events event_mask pointer_mode
  ...
  in
  setEnum8 b 1 (owner_events : bool);
  setEnum32 b 4 (win : window);
  setCard16 b 8 (mask_of_list (event_mask :  eventMask list));
  setEnum8 b 10 (pointer_mode : grabMode);
  setEnum8 b 11 (keyboard_mode : grabMode);
  setEnum32 b 12 (confine_to : window);
  setEnum32 b 16 (cursor : cursor);
  setTime b 20 time;
  b
...
let grabButtonReq win (owner_events : bool) (event_mask : eventMask list)
  pointer_mode
    keyboard_mode (confine_to : window) (cursor : cursor) 
...
let ungrabButtonReq win (button : int) modifiers =
...
let changeActivePointerGrabReq event_mask (cursor : cursor) time =
  ...
  setTime b 8 time;
  setCard16 b 12 (mask_of_list (event_mask : eventMask list));
  b
...
let grabKeyboardReq win owner_events pointer_mode keyboard_mode time =
  ...
  setEnum8 b 1 (owner_events : bool);
  setTime b 8 time;
  setEnum8 b 12 (pointer_mode : grabMode);
  setEnum8 b 13 (keyboard_mode : grabMode);
  b
...
let grabKeyReq win owner_events pointer_mode keyboard_mode key modifiers =
  ...
  in
  setEnum8 b 1 (owner_events : bool);
  setCard16 b 8 (modifiers : int);
  setCard8 b 10 (key : int);
  setEnum8 b 11 (pointer_mode : grabMode);
  setEnum8 b 12 (keyboard_mode : grabMode);
  b
