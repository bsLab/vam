...
external sema_create : 
    level:int -> semaphore = "am_sema_create"

(*
** nil semaphore structure, needed for references on sema's
*)

let nilsema = sema_create 0
...
external sema_trydown :
    semaphore -> int -> bool = "am_sema_trydown"
...
external am_sema_print :
    semaphore -> string = "am_sema_print"

let print_amoeba_sema sem =
    open_hvbox 0;
    print_string (am_sema_print sem);
    close_box ()
